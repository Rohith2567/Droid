...
class GraphDroid():
    ...
    def __train_test_split(self, exp, testonly=False):
        ...
        for d in self.train_dbs:
            logger.debug(d)
            datas = get_dataset([d], self.hop, self.tpl, self.norm_opcode, self.mask, shuffle=True)  # sample=0.3
            data_size = len(datas)
            logger.info(f'{d}: {data_size}')

            if len(datas) == 0:
                raise RuntimeError(f"No graphs found for dataset {d}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dbs', nargs='+', default=['TestOnly'])
