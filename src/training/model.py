...
def my_train(loader, test_loader, writer, model_dict, dev=None, lossfunc=0, batch_size=64, num_epoch=1000, start_epoch=0, best=None, conv_func=None, global_pool=None, train_eps=False, dimension=128, layer_norm=False):
    ...
    if osp.exists(model_dict):
        state = torch.load(model_dict, map_location=dev)

        model_input_dim = model.convs[0].nn[0].in_features
        ckpt_input_dim = state['convs.0.nn.0.weight'].shape[1]

        if model_input_dim == ckpt_input_dim:
            model.load_state_dict(state)
            model_logger.info(f"Loaded model checkpoint {model_dict}")
        else:
            model_logger.warning(
                f"Skip loading checkpoint due to feature mismatch: "
                f"{ckpt_input_dim} (ckpt) vs {model_input_dim} (current)"
            )
    ...
    try:
        ...
        for epoch in range(num_epoch):
            ...
            for batch in tqdm(loader, desc=f'Epoch {epoch}'):
                ...
                assert batch.x.size(1) == model.convs[0].nn[0].in_features
            ...
            if f1 > 0.95 or (flag and f1 > 0.85):
                ...
                savename = '%s/feat%d_%f_%f_%f_%f_%f' % (
                    model_path,
                    num_node_features,
                    precission, recall, accuracy, f1, f2
                )
...
def my_test(loader, model, dev=None, is_validation=False, curve=False, emb_=False):
    ...
    for data in loader:
        ...
        assert data.x.size(1) == model.convs[0].nn[0].in_features
