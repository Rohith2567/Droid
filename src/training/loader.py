...
MAX_NODES = 50000        # safe upper bound
MAX_EDGES = 200000      # optional but recommended
...
class GraphDroidDataset():
    ...
    def get_dataset(self, norm_opcode=False, mask=-1, sample_ratio=None):
    ...
    #     # 1️⃣ Load dataset
    #     if (not norm_opcode) and (mask == -1):
    #         dataset = torch.load(osp.join(self.data_base, 'dataset.pt'))
    #     else:
    #         if norm_opcode:
    #             dataset = self.__norm_dataset_opcode()
    #         if mask != -1:
    ...
    #     filtered_dataset = []
    #
    #     # 2️⃣ FILTER OVERSIZED API SUBGRAPHS (CRITICAL)
    #     for apk in dataset:
    #         safe_apis = []
    #         for api in apk.data:
    #             num_nodes = api.x.size(0)
    #             num_edges = api.edge_index.size(1)
    #
    #             if num_nodes > MAX_NODES or num_edges > MAX_EDGES:
    #                 continue  # ❌ skip this API subgraph
    #
    #             safe_apis.append(api)
    #
    #         # only keep APK if it still has APIs
    #         if len(safe_apis) > 0:
    #             filtered_dataset.append(Data(data=safe_apis))
    #
    #     dataset = filtered_dataset
    #
    #     # 3️⃣ Optional sampling
    #     if sample_ratio is not None:
    #         assert 0 < sample_ratio <= 1.0
    ...
    #             dataset = self.__mask_dataset(mask)
    #
    #     # 2️⃣ Apply sampling (AFTER dataset exists)
    #     if sample_ratio is not None:
    #         assert 0 < sample_ratio <= 1.0
    #         sample_size = max(1, int(len(dataset) * sample_ratio))
    #         dataset = random.sample(dataset, sample_size)
...
def real_batch(batch, max_apis=50):  # 🔴 ADD max_apis
    real = []
    position = [0]
    count = 0

    for apk in batch.data:
        apis = apk[:max_apis]  # 🔴 LIMIT APIs PER APK
        for api in apis:
            real.append(api)
        count += len(apis)
        position.append(count)
