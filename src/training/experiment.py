...
class Experiment():
    ...
    def __set_paths(self):
        ...
        columns = ['exp_id', 'trained_db', 'tpl', 'hop', 'norm_opcode', 'mask', 'feature_dim', 'model_config'] # 'feature_dim',
    ...
    def __create_exp(self, columns):
        ...
        # ✅ NEW: compute feature dimension ONCE
    ...
    def __get_exp_id(self):
        ...
        for _, v in configs.iterrows():
            ...
            current_feature_dim = self.get_feature_dim()

            match = (
                    (eval(v.trained_db) == self.trained_db) &
                    (v.tpl == self.tpl) &
                    (v.hop == self.hop) &
                    (v.norm_opcode == self.norm_opcode) &
                    (v['mask'] == self.mask) &
                    (v.feature_dim == current_feature_dim) &  # ✅ ADD
                    (eval(v.model_config) == self.model_config)
            )
            ...
            #         and v.tpl == self.tpl
            #         and v.hop == self.hop
            #         and v.norm_opcode == self.norm_opcode
            #         and v['mask'] == self.mask
            #         and v['feature_dim'] == feature_dim
            #         and eval(v.model_config) == self.model_config
            # )
    ...
    def find_best_model(self, keys=['f1']):
        ...
        for m in models:
            ...
            if not dict_name.startswith('last_epoch_'):
                ...
                # NEW FORMAT: feat{dim}_prec_recall_acc_f1_f2
                if parts[0].startswith('feat'):
                    parts = parts[1:]  # drop featXXX

                m_score = [m]
                m_score.extend(parts[:5])  # force exactly 5 metrics
                scores.append(m_score)
        ...
        for k in keys:
            ...
            values = pd.to_numeric(scores[k], errors='coerce')  #.dropna()
            indexes = values[values == values.max()].index
    ...
    def get_feature_dim(self):
        """
        Feature dimension must match the ACTUAL dataset used for training.
        This function is only called AFTER TrainTest data exists.
        """
        train_path = f'{self.exp_base}/{self.exp_id}/TrainTest/train.pt'

        if not os.path.exists(train_path):
            # Fallback for exp_id resolution BEFORE training
            # Use CSV estimate ONLY to separate experiments
            db = list(self.trained_db)[0]
            flens = get_feature_len(db, self.hop, self.tpl, base_dir=self.graph_base)
            return sum(flens)
    ...
    def load_model(self, model_path=None):
        ...
        saved_w = state['convs.0.nn.0.weight']
        saved_in_dim = saved_w.shape[1]

        current_in_dim = conv_model.convs[0].nn[0].weight.shape[1]

        if saved_in_dim != current_in_dim:
            raise RuntimeError(
                f"Model feature mismatch: saved={saved_in_dim}, current={current_in_dim}"
            )

        conv_model.load_state_dict(state)
