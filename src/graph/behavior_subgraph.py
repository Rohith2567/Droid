...
class MyOwnDataset(Dataset):
    ...
    def len(self):
        ...
        gids = [int(os.path.basename(p).split('_')[1]) for p in pt_files]

        self.samples = max(gids) + 1  # ✅ critical fix
...
def get_feature(gmlfile, base_dir, fwords=['permission','opcode','tpl'], getsep=False):
    # Extract APK hash from call.gml path
    apk_hash = os.path.basename(os.path.dirname(gmlfile))

    # Build correct feature paths
    features = [
        os.path.join(base_dir, 'result', w, f'{apk_hash}.csv')
        for w in fwords
    ]

    if getsep:
        return [pd.read_csv(f).shape[1] - 1 for f in features]

    return [pd.read_csv(f).assign(type=i+1) for i, f in enumerate(features)]
...
async def generate_behavior_subgraph(p, features, single_graph, hop, debug, gmlfile, apk_name, y):
    ...
    # ---- FIXED EDGE CONSTRUCTION (ALWAYS [2, E]) ----
    edges = subgraph_edges  # [(src, dst, key), ...]
    ...
    mapping = {
        int(row['id']): idx
        for idx, row in subgraph_features.iterrows()
    }
    ...
    # If graph becomes edge-less, skip it
    if len(src) == 0:
        return None
    ...
    assert edge_index.max().item() < subgraph_features.shape[0], \
        f"edge_index overflow: max={edge_index.max()} nodes={subgraph_features.shape[0]}"
    ...
    # mapping & center
    mapping = {int(row['id']): idx for idx, row in subgraph_features.iterrows()}
    center = mapping[int(p)]
    ...
    data = Data(x=torch.tensor(subgraph_features.iloc[:,1:-1].values.tolist(), dtype=torch.float)
                , edge_index=edge_index    # edge_index=torch.tensor(edge_list, dtype=torch.long)
...
async def gml2Data(gmlfile, y, base_dir, tpl=True, sub=True, hop=2, debug=False):
    ...
    # 🔧 FIX: normalize node id types
    permission['id'] = permission['id'].astype(str)
    opcodes['id'] = opcodes['id'].astype(str)

    if tpl:
        tpl_df = x[2]
        tpl_df['id'] = tpl_df['id'].astype(str)
    ...
    features_exist = (
        pd.merge(permission, opcodes, how='outer', on='id')
        .fillna(0)
    ...
    # 🔧 FINAL FIX: ensure 'type' column always exists
    if 'type' not in features.columns:
        features['type'] = 0  # default: normal node

    features['type'] = features['type'].astype(int)

    # OLD (breaks small datasets)
    # p_list = x[0].id.tolist()

    # use ALL nodes as centers if permission-only fails

    MAX_SUBGRAPHS = 100  # or even 50 for now

    p_list = features['id'].tolist()[:MAX_SUBGRAPHS]
    ...
    if sub:
        ...
        for p in p_list:
            data = await generate_behavior_subgraph(
                p,
                features=features,
