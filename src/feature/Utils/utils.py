...
def create_csv(smali_opcode, path):
    # Ensure directory exists (CRITICAL for Windows + threads)
    dirpath = os.path.dirname(path)
    if dirpath and not os.path.exists(dirpath):
        os.makedirs(dirpath, exist_ok=True)

    # Force UTF-8 and ignore invalid chars
    f = open(path, 'w', newline='', encoding='utf-8', errors='ignore')
    csv_write = csv.writer(f)

    # Header
    csv_head = ['id'] + list(smali_opcode)
    csv_write.writerow(csv_head)
...
def write_csv(opcode, f, id):  # 写csv
    ...
    # csv_write.writerow([str(x) for x in data_row])
