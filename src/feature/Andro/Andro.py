    ...
    def analysis_app(self, log, apkobj, dexobj, analysisobj):
        ...
        for classes in analysisobj.get_classes():  # all class
            ...
            try:
                ...
                class_name = str(classes.get_name())
            ...
            for method in classes.get_methods():
                ...
                method_fullname = get_method_fullname(m)
                class_functions[class_name].append(method_fullname)
                ...
                if flag:
                    ...
                    #     utils.write_csv(opcode, opcodeFile, method2nodeMap[method_fullname][0])
                    # except Exception:
                    #     # print("apk: %s, method: %s not exists"%(log.filename, str(m.full_name)))
                    #     # print("apk: %s, method: %s not exists" % (log.filename, method_fullname))
                    #     _settings.logger.debug("Method not in call graph: %s", method_fullname)
                    node = method2nodeMap.get(method_fullname)

                    if node is None:
                        # Safe logging (no Unicode crash)
                        _settings.logger.debug(
                            "Method not in call graph (skipped)"
                        )
                        continue  # or return, depending on loop context

                    utils.write_csv(opcode, opcodeFile, node[0])
